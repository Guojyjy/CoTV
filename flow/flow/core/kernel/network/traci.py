...
class TraCIKernelNetwork(BaseKernelNetwork):
    ...
    def generate_network(self, network, emission_path, safety_device, glosa_device):
        """See parent class.
        ...
        try:
            template_vehicles = self.network.template_vehicles
        except AttributeError:
            template_vehicles = {}

        # create the sumo configuration files
        cfg_name, random_num = self.generate_cfg(emission_path, safety_device, glosa_device,
                                                 self.network.net_params,
                                                 self.network.traffic_lights,
                                                 self.network.routes,
                                                 template_vehicles)
    ...
    def generate_cfg(self, emission_path, safety_device, glosa_device, net_params, traffic_lights, routes, vehicles):
        ...
        for route_id in routes.keys():
            # in this case, we only have one route, convert into list of routes with one element
        ...
        if self.network.net_params.template:
            for each, info in vehicles.items():
                routes_data.append(E(
                    'vehicle',
                    id=each,
                    departSpeed=info['departSpeed'],
                    depart=info['depart'],
                    type=info['typeID'],
                    departPos=info['departPos'],
                    route='route{}_{}'.format(info['routeID'], 0),
                ))
        ...
        random_num = random.randint(1, 9)  # avoid rewrite between episodes
        for each in routes_data.iter():
            if each.tag == 'flow' or each.tag == 'vehicle':  # tag: flow (grid maps), vehicle (dublin)
                ...
                if safety_device:
                    ...
                    each.append(E('param', key='device.ssm.measures', value='TTC DRAC PET BR SGAP TGAP'))
                    each.append(E('param', key='device.ssm.thresholds', value='3.0 3.0 2.0 0.0 0.2 0.5'))
                    each.append(E('param', key='device.ssm.range', value='50.0'))
                    each.append(E('param', key='device.ssm.extratime', value='5.0'))
                    each.append(E('param', key='device.ssm.file',  # save ssm.xml file with other output files
                                  value='{}/{}-{}-ssm.xml'.format(emission_path, self.name, random_num)))
                    each.append(E('param', key='device.ssm.trajectories', value='true'))
                    each.append(E('param', key='device.ssm.geo', value='false'))
