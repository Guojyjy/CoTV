...
def stable_acceleration_positive_edge(env, veh_ids):
    vel = []
    for veh in veh_ids:
        accel = env.k.vehicle.get_realized_accel(veh)
        if accel >= 0:
            if accel <= 9:
                vel.append(accel / 9)
            else:
    ...
    max_accel = np.array([1] * num_vehicles)
    max_accel = np.linalg.norm(max_accel)

    # epsilon term (to deal with ZeroDivisionError exceptions)
    eps = np.finfo(np.float32).eps

    return max(vel, 0) / (max_accel + eps)
...
def min_delay_edge(env, veh_ids, target_speed):

    vel = np.array(env.k.vehicle.get_speed(veh_ids))

    vel = vel[vel >= -1e-6]
    vel[vel > target_speed] = target_speed
    # v_top = max(
    #     env.k.network.speed_limit(edge)
    #     for edge in env.k.network.get_edge_list())
    v_top = target_speed
