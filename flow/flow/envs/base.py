...
class Env(gym.Env, metaclass=ABCMeta):
    ...
    * compute_reward
    ...
    def __init__(self,
        ...
        random_num = self.k.network.generate_network(self.network, self.sim_params.emission_path,
                                                     self.env_params.additional_params.get("safety_device", False),
                                                     self.env_params.additional_params.get("glosa_device", False))
        ...
        kernel_api = self.k.simulation.start_simulation(
            network=self.k.network, sim_params=self.sim_params, random_num=random_num)
    ...
    def reset(self):
        ...
        if len(self.initial_ids) > len(initial_ids):
            ...
            msg = '\nNot enough vehicles have spawned at the beginning of this episode!\n' \
            ...
            print(msg)
            print(
                "Cuz: add vehicles instead of inflow, and the initial position-the end of edge in flow/core/kernel/vehicle/traci")
    ...
    @property
    # remove @abstractmethod:  Can't instantiate abstract class MultiCAVTLCustomEnv
    # with abstract methods action_space, observation_space
